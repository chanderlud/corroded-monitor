...
use iced::Element;
use iced::widget::{Button, PickList, Column, Container, Row, Space, Text};
use iced_style::theme;
use serde_json::Value;

use crate::Data;
use crate::ui::{chart::{Size, StatChart}, Message, Route};
...
impl Cpu {
    ...
    pub fn view(&self) -> (Element<Message>, Element<Message>) {
        ...
        let graphs = match self.graph_state {
            GraphState::Utilization => self.cores.iter().map(|c| Element::new(Container::new(c.load_graph.view()).style(theme::Container::Custom(Box::new(GraphBox { color: (0, 255, 255) }))).width(Length::FillPortion(1)).height(Length::FillPortion(1)))).collect::<Vec<Element<Message>>>(),
            GraphState::Temperature => self.cores.iter().map(|c| Element::new(Container::new(c.temperature_graph.view()).style(theme::Container::Custom(Box::new(GraphBox { color: (183, 53, 90) }))).width(Length::FillPortion(1)).height(Length::FillPortion(1)))).collect::<Vec<Element<Message>>>(),
            GraphState::Frequency => self.cores.iter().map(|c| Element::new(Container::new(c.frequency_graph.view()).style(theme::Container::Custom(Box::new(GraphBox { color: (255, 190, 125) }))).width(Length::FillPortion(1)).height(Length::FillPortion(1)))).collect::<Vec<Element<Message>>>()
        };
        ...
        let large = Column::new().padding(20)
            .push(
                Row::new().align_items(Alignment::Center).height(Length::Fixed(30.0))
                    ...
                    .push(PickList::new(&GraphState::ALL[..], Some(self.graph_state), Message::CpuPickChanged).style(theme::PickList::Custom(Rc::new(PickListStyle), Rc::new(PickListStyle))))
